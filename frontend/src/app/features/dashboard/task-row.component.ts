...
@Component({
  ...
  template: `
    <div class="group flex items-center gap-2 rounded-[6px_9px_5px_8px/8px_5px_9px_6px]
      ...
      @if (todayLabel()) {
        <span class="text-muted shrink-0 text-[13px]">{{ todayLabel() }}</span>
      }
      <button type="button"
              class="btn shrink-0 px-2.5 py-1 text-xs opacity-100 transition-opacity
                     sm:opacity-0 sm:group-hover:opacity-100 sm:group-focus-within:opacity-100 focus:opacity-100"
              (click)="log.emit()">+ log</button>
      @if (isTracking()) {
        <button type="button" class="btn danger shrink-0 px-2.5 py-1 text-xs" (click)="stop.emit()">■ stop</button>
      } @else {
        <button type="button"
                class="btn shrink-0 px-2.5 py-1 text-xs opacity-100 transition-opacity
                       sm:opacity-0 sm:group-hover:opacity-100 sm:group-focus-within:opacity-100 focus:opacity-100"
