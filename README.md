...
A donut chart of how your time splits across tasks over the last week or month, with the window's total in the middle and a name / duration / share breakdown beside it — plus each task's change vs the preceding window (`new` where there's no baseline).
...
- **Task-share donut** — a Chart.js doughnut of each task's share of your time over the last week or month, with per-task durations, percentages, and change vs the preceding window.
